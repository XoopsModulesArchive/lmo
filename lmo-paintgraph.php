...
$breit = (($pgst + 1) * 12) + 35;
$vergleich = imagefontwidth(3) * mb_strlen(stripslashes($pgteam1)) + 3;
if (2 == $pganz) {
    $vergleich += imagefontwidth(3) * mb_strlen(stripslashes($pgteam2)) + 4;
}
if ($breit < $vergleich) {
    $breit = $vergleich;
}
...
for ($i = 0; $i < $pgteams; $i++) {
    imagerectangle($image, 29, 28 + ($i * 12), 17 + (($pgst + 1) * 12), 40 + ($i * 12), $farbe_b);
}
for ($i = 0; $i < $pgst; $i++) {
    imagerectangle($image, 29 + ($i * 12), 28, 41 + ($i * 12), 16 + (($pgteams + 1) * 12), $farbe_b);
}
$j = 1;
for ($i = 1; $i <= $pgteams; $i++) {
    if ((1 == $i) && (0 != $pgch)) {
        ...
        for ($k = 1; $k <= $pgst; $k++) {
            imagefill($image, 20 + ($k * 12), 20 + ($i * 12), $farbe_e);
        }
    }

    if (($i >= $j) && ($i < $j + $pgcl) && ($pgcl > 0)) {
        for ($k = 1; $k <= $pgst; $k++) {
            imagefill($image, 20 + ($k * 12), 20 + ($i * 12), $farbe_f);
        }
    }

    if (($i >= $j + $pgcl) && ($i < $j + $pgcl + $pgck) && ($pgck > 0)) {
        for ($k = 1; $k <= $pgst; $k++) {
            imagefill($image, 20 + ($k * 12), 20 + ($i * 12), $farbe_g);
        }
    }

    if (($i >= $j + $pgcl + $pgck) && ($i < $j + $pgcl + $pgck + $pguc) && ($pguc > 0)) {
        for ($k = 1; $k <= $pgst; $k++) {
            imagefill($image, 20 + ($k * 12), 20 + ($i * 12), $farbe_h);
        }
    }

    if (($i <= $pgteams) && ($i > $pgteams - $pgab) && ($pgab > 0)) {
        for ($k = 1; $k <= $pgst; $k++) {
            imagefill($image, 20 + ($k * 12), 20 + ($i * 12), $farbe_i);
        }
    }

    if (($i <= $pgteams - $pgab) && ($i > $pgteams - $pgab - $pgar) && ($pgar > 0)) {
        for ($k = 1; $k <= $pgst; $k++) {
            imagefill($image, 20 + ($k * 12), 20 + ($i * 12), $farbe_j);
        }
    }
}
