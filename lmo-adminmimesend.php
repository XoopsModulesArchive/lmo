...
if (isset($HTTP_SESSION_VARS['lmousername'])) {
    $lmousername = $HTTP_SESSION_VARS['lmousername'];
}
if (isset($HTTP_SESSION_VARS['lmouserpass'])) {
    $lmouserpass = $HTTP_SESSION_VARS['lmouserpass'];
}
if (isset($HTTP_SESSION_VARS['lmouserfile'])) {
    $lmouserfile = $HTTP_SESSION_VARS['lmouserfile'];
}
if (('admin' == $action) && ('email' == $todo) && ((1 == $HTTP_SESSION_VARS['lmouserok']) || (2 == $HTTP_SESSION_VARS['lmouserok']))) {
    if ((0 != $down) && ('' != $madr)) {
        $array = [''];
        ...
        if ($down > 0) {
            if ('' != $dummy[$down - 1]) {
                ...
                $mail->add_attachment($zipfile->file(), 'lmo_' . mb_substr($dummy[$down - 1], 0, -4) . '.zip', 'application/octet-stream');
            ...
            if (count($dummy) > 0) {
                ...
                for ($i = 0, $iMax = count($dummy); $i < $iMax; $i++) {
